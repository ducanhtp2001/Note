...
import com.example.note.Tools.EditTextFlow
...
class PostEditBottomSheetFragment : BaseBottomSheetDialogFragment() {
    ...
    private fun updatePost(p: Post?) {
        ...
        tryWithLog {
            p?.imgOnBitmap?.let { bitmap ->
                Glide.with(requireContext()).load(bitmap).into(binding.btnImage)
            }
        }
    }
    ...
    private fun initView() {
        viewModel.post.value?.let {
            binding.edNewPost.setText(it.noiDung ?: "")
            binding.isHasImage = it.coImg == 1
            tryWithLog {
                it.imgOnBitmap?.let { bitmap ->
                    Glide.with(requireContext()).load(bitmap).into(binding.btnImage)
                }
            }
        }
