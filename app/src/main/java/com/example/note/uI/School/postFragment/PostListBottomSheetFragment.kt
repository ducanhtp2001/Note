...
class PostListBottomSheetFragment : BaseBottomSheetDialogFragment(), BaseHandler by BaseHandlerImpl(), PostAdapter.PostListener {
    ...
    private fun setupViewEvents() {
        ...
        binding.cardNewPost.setOnClickListener {
            (openPostEditBottomSheet(requireActivity()) as? Dialog)?.setOnDismissListener {
                viewModel.getPostList()
            }
        }
    }
    ...
    override fun showPostClick(post: PostResult, position: Int) {
        if (AppState.getInstance().isAppUser(post.info.id)) {
            AppState.getInstance().selectedPost = post
            openPostEditBottomSheet(requireActivity())
        } else toaster.display(getString(R.string.common_user_not_permission_edit_post))
    }
